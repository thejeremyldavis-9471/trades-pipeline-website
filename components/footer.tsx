...
export function Footer({ variant = "residential" }: FooterProps) {
  ...
  const accent = isCommercial ? "text-slate-300" : "text-[#FF6B2B]";
    ...
    <footer className="bg-[#0D1117] border-t border-slate-800 py-14 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10 mb-10">
          <div>
            <div className="font-display font-bold text-white text-xl tracking-wider uppercase mb-3">The Trades<span className={accent}> Pipeline</span></div>
            <p className="text-slate-400 text-sm leading-relaxed mb-4">Built for the Trades.<br />Wired for Growth.</p>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {["Website Design", "Local SEO & GBP", "Social Media", "Paid Advertising", "Residential Lead Gen", "B2B Commercial"].map((s) => (<li key={s}><Link href="#" className="text-slate-400 hover:text-white text-sm transition-colors">{s}</Link></li>))}
            </ul>
          </div>
          <div>
            <h4 className="text-white font-bold text-xs tracking-[0.25em] uppercase mb-5">Company</h4>
            <ul className="space-y-2.5">
              {[["Home", "/"], ["Residential", "/residential"], ["Commercial", "/commercial"], ["Contact", "/contact"]].map(([label, href]) => (<li key={href}><Link href={href} className="text-slate-400 hover:text-white text-sm transition-colors">{label}</Link></li>))}
            </ul>
          </div>
          <div>
            ...
            <Link href="/contact"><button className={`w-full ${ctaClass} font-bold py-3 text-xs tracking-widest uppercase transition-colors`}>Book Strategy Call</button></Link>
          </div>
        </div>
